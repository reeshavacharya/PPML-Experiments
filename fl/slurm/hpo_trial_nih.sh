...
#SBATCH --output=/work/r/reeshav/PPML-Experiments-FL/fl/slurm/std_out_hpo_trial_nih_%j.log
#SBATCH --error=/work/r/reeshav/PPML-Experiments-FL/fl/slurm/std_err_hpo_trial_nih_%j.log

# Combined server + all-clients job for one NIH FL HPO trial, sharing a
# single GPU — replaces the old 2-job layout (CPU-only server job +
# 2-array-task client job needing 2 GPUs at once). fl/client_nih.py already
...
# clients on one GPU — packing all 5 onto 1 GPU instead of 3+2 across 2
...
# HP_CONFIG, RESULT_PATH, HPO_ROUNDS, NUM_CLIENTS (default 5),
...
echo "=== NIH FL HPO Trial ${TRIAL_ID} (single-GPU) starting on $(hostname) at $(date) ==="
...
python3 -u fl/server_nih.py \
    ...
    > "fl/slurm/std_out_hpo_server_nih_trial_${TRIAL_ID}.log" 2>&1 &
CHILD_PIDS+=($!)
...
for CID in $(seq 1 "${NUM_CLIENTS}"); do
    python3 -u fl/client_nih.py \
        ...
        "${SUBSAMPLE_ARGS[@]}" \
        > "fl/slurm/std_out_hpo_client_nih_${CID}_trial_${TRIAL_ID}.log" 2>&1 &
    CHILD_PIDS+=($!)
...
wait "${CHILD_PIDS[@]}"
echo "=== NIH FL HPO Trial ${TRIAL_ID} finished at $(date) ==="
