...
from data_loader.isic_dataset import NUM_CLASSES, NUM_CLIENTS
...
def evaluate_metrics_aggregation_fn(results):
    ...
    weighted = sum(n * m.get("val_balanced_acc", 0) for n, m in results)
    return {"val_balanced_acc": weighted / total if total else 0}


def main():
    parser = argparse.ArgumentParser(description="Flower Server for ViT-B/16 Fed-ISIC2019 FL experiments (plain FedAvg)")
    parser.add_argument("--server_address", type=str, default="0.0.0.0:8081")
    ...
    parser.add_argument("--result_path", type=str, default=None,
                        help="Path to write trial result JSON {mean_balanced_acc, best_round} for HPO coordinator")
    ...
    resume_filename = f"fl_resume_isic{seed_suffix}.pth"
    best_model_filename = f"vit_isic_fl{seed_suffix}_best.pth"
    ...
    if args.resume and os.path.exists(resume_path):
        ...
        print(f"[{_ts()}]   Best val balanced accuracy: {best_metric:.4f} (round {best_round})", flush=True)
    elif args.resume:
    ...
    print("=" * 60, flush=True)
    print(f"[{_ts()}] FedAvg Server (isic) — {platform.node()}", flush=True)
    ...
    # ISIC-specific handoff file (not the shared fl/server_address.txt used by
    ...
    # seriously) both servers were bound to the same default port 8080,
    # so a client could receive a completely incompatible model checkpoint
    ...
    addr_file = os.path.join(project_root, "fl", "server_address_isic.txt")
    ...
    strategy = CASFedAvgStrategy(
        ...
        metric_key="val_balanced_acc",
        metric_label="balanced_acc",
        log_label="ISICFedAvgStrategy: plain FedAvg (6 non-IID centers)",
    ...
    summary_path = os.path.join(project_root, "fl", f"training_summary_isic{seed_suffix}.json")
    ...
    if args.result_path:
        ...
        with open(args.result_path, "w") as f:
            json.dump({
                "mean_balanced_acc": summary["best_val_balanced_acc"],
    ...
    print(f"  Best Val Balanced Accuracy: {summary['best_val_balanced_acc']} (round {summary['best_round']})", flush=True)
