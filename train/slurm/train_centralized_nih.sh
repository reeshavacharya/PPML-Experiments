...
#SBATCH --output=/work/r/reeshav/PPML-Experiments-FL/train/slurm/std_out_train_baseline_nih.log
#SBATCH --error=/work/r/reeshav/PPML-Experiments-FL/train/slurm/std_err_train_baseline_nih.log
...
# Read tuned lr/weight_decay from hpo_best_centralized_nih.json (written by
# train/hpo_centralized.py --dataset nih) if it exists, so this script always
# trains with HPO-tuned hyperparameters once available, with no manual
# copy-paste step. Falls back to centralized_multilabel.py's own argparse
# defaults (with a clear warning) if HPO hasn't completed yet.
HPO_JSON="hpo_best_centralized_nih.json"
EXTRA_ARGS=()
if [ -f "$HPO_JSON" ]; then
...
srun python3 train/centralized_multilabel.py \
    --dataset nih \
    --epochs 50 \
    --batch_size 32 \
    --loss bce \
